...
ローカル検証環境であり、本番分散環境で問われる論点の多くには踏み込めていないが、ストリーミング基盤を組む上での所在は意識して構築している。

### 検証の過程で意識した論点

- **Exactly-once セマンティクス**：Flinkのチェックポイント（10秒間隔で有効化）と、Iceberg sink のチェックポイント単位コミットを連動させ、ストリームからテーブルへの書き込みで重複・欠損を抑える構成とした。一方で、Kafkaソース側の厳密なオフセット管理（エンドツーエンドの保証）までは踏み込めておらず、ソースを含めた一貫した Exactly-once は今後の検証対象。

### 今後の発展方向（未着手）

- **遅延データ・順序保証**：現構成は処理時刻ベースで、イベントタイムのウォーターマーク設計や遅延イベントの扱いには未対応。イベント時刻を基準としたストリーム処理は今後の課題。
- **スケールと障害耐性**：単一ノード・パーティション1・並列度1の構成から、パーティショニング・並列度・障害復旧を前提とした構成への発展。
